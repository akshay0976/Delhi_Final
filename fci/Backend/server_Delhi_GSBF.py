...
def extract_db():
    if request.method == 'POST':
        ...
        if connection.is_connected():
            ...
            if user:
                for row in user:
                    temp = {'State Name':'','WH_District': row[0], 'WH_Name': row[1], 'WH_ID': row[2], 'Type of WH': row[3], 'WH_Lat': row[5], 'WH_Long': row[6], 'Quantity_Wheat': row[7], 'Quantity_FRice': row[8],'Owned/Rented':'', 'quantity of Wheat stored (Quintals)':''}
...
def fci_data():
    try:
        ...
        total_supply_wheat = float(fci['Quantity_Wheat'].sum())
        total_supply_frice = float(fci['Quantity_FRice'].sum())

        result = {'Warehouse_No': warehouse_no, 'FPS_No': fps_no, 'Total_Demand': total_demand,'Total_Demand_Rice': total_demand_rice, 'Total_Supply_Wheat': total_supply_wheat,'Total_Supply_Frice': total_supply_frice, 'District_Count': districts_no}
    ...
    try:
        ...
        for i in range(len(FCI["WH_District"])):
            ...
            if District_Name not in District_Capacity:
                District_Capacity[District_Name] = float(FCI["Quantity_Wheat"][i])
            else:
                District_Capacity[District_Name] += float(FCI["Quantity_Wheat"][i])

        District_Capacity_FRice = {}
        for i in range(len(FCI["WH_District"])):
            District_Name = FCI["WH_District"][i]
            if District_Name not in District_Capacity_FRice:
                District_Capacity_FRice[District_Name] = float(FCI["Quantity_FRice"][i])
            else:
                District_Capacity_FRice[District_Name] += float(FCI["Quantity_FRice"][i])
        ...
        combined_data = {'District_Demand': District_Demand, 'District_Capacity': District_Capacity, 'District_Capacity_FRice': District_Capacity_FRice,'District_Name': District_Name_1,'District_Demand_Rice': District_Demand_Rice,}
...
def save_to_database(month, year, applicable):
    ...
    if connection.is_connected():
        ...
        create_fps_query = ("CREATE TABLE " + fps_table + " (district VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, id VARCHAR(100) NOT NULL, type VARCHAR(100) NOT NULL, latitude VARCHAR(100) NOT NULL,longitude VARCHAR(100) NOT NULL,Allocation_Wheat VARCHAR(100) NOT NULL, uniqueid VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1',Allocation_Rice VARCHAR(100) NOT NULL)")
...
def processFile():
    ...
    write_log("User -> " + "Leg 2 Optimisation Start Requested JSON -> " +str(json_data))
    ...
    #    Variables for Wheat from IG TO FPS
        ...
        for i in range(len(FCI["WH_District"])):
            District_Name = FCI["WH_District"][i]
            if District_Name not in District_Capacity:
                District_Capacity[District_Name] = int(FCI["Quantity_Wheat"][i])
            else:
                District_Capacity[District_Name] += int(FCI["Quantity_Wheat"][i])
        ...
        for i in range(len(FCI["WH_District"])):
            ...
            if District_Name not in District_Capacity1:
                District_Capacity1[District_Name] = int(FCI["Quantity_FRice"][i])
            else:
                District_Capacity1[District_Name] += int(FCI["Quantity_FRice"][i])
        ...
        for i in range(len(FPS["FPS_District"])):
            District_Name_FPS = FPS["FPS_District"][i]
            if District_Name_FPS not in District_Demand1:
                District_Demand1[District_Name_FPS] = float(FPS["Allocation_Wheat"][i]) 
            else:
                District_Demand1[District_Name_FPS] += float(FPS["Allocation_Wheat"][i]) 
        ...
        District_Name1 = []
        District_Name21=[]
        District_Name1 = [i for i in District_Demand1 if i not in District_Capacity1]
        District_Name41 = [i for i in District_Capacity1 if i not in District_Demand1]
        District_Name21 = [i for i in District_Demand1 if i in District_Capacity1 and District_Demand1[i] >= District_Capacity1[i]]
        District_Name_11 = {}
        District_Name_11['District_Name_All'] = District_Name1 + District_Name21
        District_Name31 = [i for i in District_Demand1 if i in District_Capacity1 and District_Demand1[i] <= District_Capacity1[i]]
        ...
        lst5 = []
        for j in range(len(DV_Variables1)):
            name5 = str(DV_Variables1[j])
            lst5 = name5.split("_")
            if lst5[2] in District_Name31 and lst5[4] in District_Name31 and lst5[2]!=lst5[4]:
                model+=DV_Variables1[j]==0
                #print(DV_Variables1[j]==0)
        ...
        lst6 = []
        for j in range(len(DV_Variables1)):
            name6 = str(DV_Variables1[j])
            lst6 = name6.split("_")
            if lst6[2] in District_Name21 and lst6[4] in District_Name31:
                model+=DV_Variables1[j]==0
                #print(DV_Variables1[j]==0)
        ...
        lst8 = []
        for j in range(len(DV_Variables1)):
            name8 = str(DV_Variables1[j])
            lst8 = name8.split("_")
            if lst8[2] in District_Name21 and lst8[4] in District_Name21 and lst8[2]!=lst8[4]:
                model+=DV_Variables1[j]==0
                #print(DV_Variables1[j]==0)
        ...
        lst7 = []
        for j in range(len(DV_Variables1)):
            name7 = str(DV_Variables1[j])
            lst7 = name7.split("_")
            if lst7[2] in District_Name41 and lst7[4] in District_Name31:
                model+=DV_Variables1[j]==0
                #print(DV_Variables1[j]==0)
       ...
       # Calling CBC_CMB Solver
        ...
        df51.insert(2, 'From_State', 'Delhi')
        df51.insert(7, 'To', 'FPS')
        df51.insert(8, 'To_State', 'Delhi')
        ...
        with pd.ExcelWriter('Backend//Delhi_Distance_L2.xlsx') as writer:
            Distance3.to_excel(writer, sheet_name='BG_BG',index=False)
            

        Cost = pd.ExcelFile("Backend//Delhi_Distance_L2.xlsx")
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        files_to_delete = [
            'Backend/Delhi_Distance_L2.xlsx',
...
def processFile_leg1():
    ...
    stop_process = False
    json_data = request.form
    write_log("User -> " + "Leg 1 Optimisation Start Requested JSON -> " + str(json_data))
    json_data = request.form
    scenario_type = request.form.get('type')
    '''scenario_type="intra"'''
    if scenario_type == "intra":
