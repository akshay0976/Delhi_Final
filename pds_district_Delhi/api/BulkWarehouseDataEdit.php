...
$mapData = [
    ...
    "Latitude" => "latitude",
    "Longitude" => "longitude",
    "Wheat Quantity" => "storage",
	"FRice Quantity" => "storage1",
	...
	//if (isset($_POST["submit"])){
		...
		if ($_FILES["file"]["size"] > 0) {
			...
			while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
				if($i>0){
					if($district<0 or $name<0 or $id<0 or $type<0 or $storage<0 or $storage1<0 or $latitude<0 or $longitude<0 or $warehousetype<0 or $active<0){
					...
					if(!isStringNumber($column[$storage])){
						echo "Error : Check Wheat Quantity Value: ".$column[$storage];
						echo "</br>";
						$redirect = 0;
					}
					if(!isStringNumber($column[$storage1])){
						echo "Error : Check Rice Quantity Value: ".$column[$storage1];
						echo "</br>";
						$redirect = 0;
					}
					...
					$Warehouse->setStorage1($column[$storage1]);
